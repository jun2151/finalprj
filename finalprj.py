...
def test(epoch, phase):
  ...
  with torch.no_grad():
    ...
    for inputs, labels in dataloaders[phase]:
      ...
      if (phase == 'test'):
        print('%#{} pred,label'.format(cnt))
        print(preds.tolist(), labels.data.tolist())
        print(outputs[0].tolist()) #[0.4377, 0.1350]
        #add for ensemble 
        outputs_ensemble.append(outputs[0].tolist())
...
parser.add_argument('--path-kind', type=str, default='ds/hq/f2f' )
args = parser.parse_args()

num_epochs = args.num_epochs
model_type = args.model_type
path_kind = args.path_kind
print('#num_epochs,model_type,path_kind: {}, {}, {}'.format(num_epochs, model_type, path_kind))
...
path_test = 'ds/video' 
print("#path_train,val,test: {}, {}, {}".format(path_train, path_val, path_test))
...
dataloaders['valid'] = torch.utils.data.DataLoader(ds_val,
  batch_size=batch_size, shuffle=False,
  num_workers=0)
dataloaders['test']  = torch.utils.data.DataLoader(ds_test,
  #batch_size=batch_size, shuffle=False,
...
#ensemble process add 
if (model_type=='ensemble'):
  print('\n#ensemble processing')
  outputs_ensemble_ef = outputs_ensemble
  outputs_ensemble = []
 
  ### run xception for ensemble
  model = Xception().to(device)
  criterion = nn.CrossEntropyLoss()
  optimizer = optim.SGD(model.parameters(), lr = 0.05, momentum=0.9, weight_decay=1e-4)
  ...
  print_confusion_matrix()
  outputs_ensemble_xc = outputs_ensemble
  
  outputs_ensemble = (np.array(outputs_ensemble_ef) + np.array(outputs_ensemble_xc) )/2
  
  preds_numpy =np.argmax(outputs_ensemble, axis= 1)
  epoch_t_preds = preds_numpy.tolist()
  print('\n#confusion_matrix for ensemble')
  print_confusion_matrix()

  print('\n#outputs_ensemble')
  cnt = 0
  for output in outputs_ensemble:
    cnt = cnt + 1
    print("{} {}  {:.4f}  {:.4f}".format(cnt, epoch_t_labels[cnt-1], output[0], output[1]))
